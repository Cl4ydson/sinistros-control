...
class ESinistrosRepository:
    ...
    def salvar_ou_atualizar(self, dados: Dict) -> bool:
        ...
        try:
            ...
            logger.info(f"Salvando/atualizando: NF={nota_fiscal}, Conh={nr_conhecimento}")
    ...
    def _mapear_campos(self, dados: Dict) -> Dict:
        ...
        mapeamento = {
            ...
            # NEW STANDARDIZED COLUMNS - Payment section (nomes corretos das colunas)
            'numero_nd': 'NUMERO ND',
            'data_vencimento_nd': 'DATA VENCIMENTO ND', 
            'observacoes_pagamento': 'OBSERVACOES PAGAMENTO',
            
            # NEW STANDARDIZED COLUMNS - Internal use section (nomes corretos das colunas)
            'observacoes_internas': 'OBSERVACOES INTERNAS',
            'responsavel_interno': 'RESPONSAVEL INTERNO',
            'data_liberacao': 'DATA LIBERACAO',
            'setor_recebimento': 'SETOR RECEBIMENTO',
            
            # NEW STANDARDIZED COLUMNS - Legal section (nomes corretos das colunas)
            'custas_juridicas': 'CUSTAS JURIDICAS',
            'status_juridico': 'STATUS JURIDICO', 
            'data_abertura_juridico': 'DATA ABERTURA JURIDICO',
            
            # NEW STANDARDIZED COLUMNS - Insurance section (nomes corretos das colunas)
            'nome_seguradora': 'NOME SEGURADORA',
            'status_seguradora': 'STATUS SEGURADORA',
            'data_abertura_seguradora': 'DATA ABERTURA SEGURADORA',
            'programacao_indenizacao_seguradora': 'PROGRAMACAO INDENIZACAO SEGURADORA',
            
            # NEW STANDARDIZED COLUMNS - Salvage section (nomes corretos das colunas)
            'programacao_pagamento_salvados': 'PROGRAMACAO PAGAMENTO SALVADOS',
            
            # NEW STANDARDIZED COLUMNS - Indemnification (nomes corretos das colunas)
    ...
    def _validar_tipos(self, dados: Dict) -> Dict:
        ...
        for campo, valor in dados.items():
            try:
                if valor is None:
                ...
                elif campo in ['nota_fiscal', 'nr_conhecimento', 'cliente', 'remetente', 'destinatario', 
                              'tipo_ocorrencia', 'descricao_ocorrencia', 'status_sinistro', 'referencia']:
                    # Campos que devem ser string
                    dados_validados[campo] = str(valor) if valor is not None else None
                elif campo in ['valor_sinistro', 'valor_nota_fiscal', 'valor_frete', 'salvados', 
                              'indenizados', 'prejuizo', 'diferenca']:
                    ...
                    if isinstance(valor, (int, float)):
                        ...
                        dados_validados[campo] = 0.0
                elif campo in ['qnt_produtos', 'mes', 'ano', 'quantidade_parcelas_indenizacao', 
                    ...
                    if isinstance(valor, int):
                        ...
                        try:
                            dados_validados[campo] = int(valor)
                        except ValueError:
                            logger.warning(f"Valor inválido para campo inteiro {campo}: {valor}")
                            dados_validados[campo] = 0
                    else:
                        dados_validados[campo] = 0
                elif campo in ['rnc_retornado', 'juridico_acionado', 'seguro_acionado', 'concluido', 
                              'vendido', 'indenizado']:
                    # Campos booleanos
                    ...
                    # Outros campos - manter como string
                    dados_validados[campo] = str(valor) if valor is not None else None
