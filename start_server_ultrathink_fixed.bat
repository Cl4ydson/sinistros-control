...
REM Verificar se a porta 8003 está livre
for /f "tokens=5" %%a in ('netstat -ano ^| findstr :8003 ^| findstr LISTENING') do set PID_BACKEND=%%a
if defined PID_BACKEND (
    echo ❌ Porta 8003 já está em uso pelo processo PID %PID_BACKEND%.
...
echo.
echo 🚀 Iniciando Backend API na porta 8003...
start "BACKEND - API Sinistros CORRIGIDA" cmd /c "call venv\Scripts\activate.bat && uvicorn app.main:app --host 0.0.0.0 --port 8003 --reload && pause"
...
echo    API Test: http://localhost:8003/
echo.
...
powershell -command "try { $r = Invoke-WebRequest -Uri 'http://localhost:8003/' -UseBasicParsing -TimeoutSec 5; Write-Host '✅ Backend API: ONLINE' } catch { Write-Host '⚠️  Backend API: Verificando...' }"
